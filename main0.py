...
async def cmd_container(update, context):
    ...
    for c in containers:
        line = f"<b>{c.name}</b> - {c.status}"
        text_lines.append(line)

        buttons.append([
            InlineKeyboardButton("Logs", callback_data=f"logs:{c.name}"),
            InlineKeyboardButton("Stream", callback_data=f"stream:{c.name}"),
            InlineKeyboardButton("Status", callback_data=f"status:{c.name}")
        ])
...
async def on_button_click(update, context):
    ...
    try:
        action, container = query.data.split(":", 1)
    except ValueError:
        await query.edit_message_text("❌ Invalid action.")
        return

    if action == "logs":
        await cmd_logs(update, context, container)
    elif action == "stream":
        await cmd_stream(update, context, container)
    elif action == "status":
        await cmd_status(update, context, container)
